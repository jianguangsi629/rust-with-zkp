...
impl ZKP {
    ...
    pub fn solver(&self, k: &BigUint, c: &BigUint, x: &BigUint) -> BigUint {
        if *k >= c * x {
...
mod tests {
    ...
    fn test_toy_example() {
        ...
        let s_fake = zkp.solver(&k, &c, &x_fake);
    ...
    fn test_toy_example_with_random_numbers() {
        ...
        let x = BigUint::from(6u32);
        let k = ZKP::generate_random_below(&q); // random number

        let c = ZKP::generate_random_below(&q); // random number
